...
class PyBullet:
    ...
    def render(
        ...
        if mode == "rgb_array":
            ...
            return px
    def get_joints_names_and_ids(self, body: str):
        joint_list = {}
        number_of_joints = p.getNumJoints(self._bodies_idx[body])
        for joint_number in range(number_of_joints):
            info = p.getJointInfo(self._bodies_idx[body], joint_number)
            joint_list[info[1]]=info[0]
        return joint_list

    def get_links_names_and_ids(self, body: str):
        link_list = {p.getBodyInfo(self._bodies_idx[body])[0].decode('UTF-8'):-1,}
        for _id in range(p.getNumJoints(self._bodies_idx[body])):
            _name = p.getJointInfo(self._bodies_idx[body], _id)[12].decode('UTF-8')
            link_list[_name] = _id
        return link_list

    def get_link_shape(self, body:str, link_index=-1):
        """Get shape name and dimension of the link.
        Args:
    ...
    def set_base_velocity(self, body: str, linearVelocity: np.ndarray, angularVelocity: np.ndarray):
        "Set base velocity"
        self.physics_client.resetBaseVelocity(self._bodies_idx[body], linearVelocity=linearVelocity, angularVelocity=angularVelocity)
        return None
    ...
    def get_link_position(self, body: str, link: int) -> np.ndarray:
        ...
        """
        if link==-1:
            position = self.get_base_position(body)
        else:
    ...
    def set_kinematic_constraint(self, constraint_name: str, parent_name: str, parent_link: int, 
                                        child_name: str, child_link: int, jointType: int,
                                        jointAxis = [0,0,0], parentFramePosition=[0,0,0], childFramePosition=[0,0,0]):
        constraint_id = self.physics_client.createConstraint(self._bodies_idx[parent_name], parent_link, self._bodies_idx[child_name], child_link, jointType=jointType,
                                                jointAxis = jointAxis, parentFramePosition= parentFramePosition , childFramePosition=childFramePosition)
        self._constraint_id[constraint_name] = constraint_id
    
    def delete_kinematic_constraint(self, constraint_name):
        "Delete a constraint set by set_kinematic_constraint"
        self.physics_client.removeConstraint(self._constraint_id.pop(constraint_name))
    ...
    def delete_object(self, body_name):
