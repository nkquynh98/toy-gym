...
class ToyPickPlaceTAMP(RobotTaskEnv):
    def __init__(self, render: bool = False, json_data = None, domain = None, problem = None, goal = None, verbose=False, enable_physic=True) -> None:
        #object_positions = [[-1.0, -1.3], [-1.0, -3] , [-1.0, -1.9], [-1.0, -2.2], [-1.0, -0.70]]

        self.map_size= json_data["_map"]["_map_dim"]
        self.problem = problem
        self.domain = domain
        self.env_name = "ToyPickPlaceTAMP_" + json_data["_map"]["_map_name"]+"_"+self.problem.domain_name
        self.verbose = verbose
        print("Env_name", self.env_name)
        sim = PyBullet(render=render,background_color=np.array([0,0,230]))
        # Must declare the Robot before the task
        ...
        robot = Toy_TAMP(sim, base_position=robot_init_pos, base_rotation=robot_init_rot, verbose=verbose, enable_physic=enable_physic)
        if goal is not None:
            task_goal = self.process_goal(goal)
        if problem is not None:
            task_goal = self.process_goal(problem.get_dict()["positive_goal"])
        
        # task_goal = {"object_0": "target_1", "object_1": "target_2", "object_2": "target_0", "object_3": "target_4", "object_4": "target_3"}
        task = PickAndPlace2D_TAMP(sim, robot=robot, json_data = json_data, goal = task_goal, verbose=verbose)
    ...
    def gripper_to_object(self, object_name):
        gripper_position = self.sim.get_link_position(self.robot.body_name, self.robot.robot_links['gripper'])[0:2]
        return gripper_position-self.sim.get_base_position(object_name)[0:2]

    def object_to_target(self, object_name, target_name):
        return self.sim.get_base_position(object_name)[0:2]-self.sim.get_base_position(target_name)[0:2]
